...
<div class="alert alert-primary" role="alert">
            ...
            <div class="col-sm">
                ...
                <div class="input-group mb-3">
                    <div class="input-group-prepend">
                        <label class="input-group-text" for="dispositivosDeAudio"><i class="fas fa-microphone"></i></label>
                    </div>
                    <select class="custom-select" name="dispositivosDeAudio" id="dispositivosDeAudio"></select>
                </div>
                <!---->
            </div>
            <div class="col-sm">
                ...
                <div class="input-group mb-3">
                    ...
                    <select class="custom-select"  name="dispositivosDeVideo" id="dispositivosDeVideo"></select>
                </div>
                <!---->
            </div>
        </div>
        <div class="row">
            <div class="col-sm">
            <center><video class="img-fluid"  id="video"></video></center>
            
                <div  class="d-flex align-items-center">
                    <strong id="duracion"></strong>
                </div>
                <div class="btn-group" role="group" aria-label="Basic example">
                    <button id="btnComenzarGrabacion" type="button" class="btn btn-primary">Grabar</button>
                    <button id="btnDetenerGrabacion" style="display:none" type="button" class="btn btn-danger">Detener y Subir</button>
                    <button onclick="window.location.href='<?=base_url('evidencia_auto_foto/').$this->uri->segment(2).'/'.$this->uri->segment(3)?>'" type="button" class="btn btn-secondary">Tomar foto</button>
            ...
            <!--<script src="<?=base_url('assets/js/script-video.js')?>"></script>-->
            <script>
                                /**
                ...
                const init = () => {
                    ...
                    const refrescar = () => {
                        ...
                        if($duracion.textContent =="00:00:10"){
                    ...
                    const comenzarAGrabar = () => {
                        ...
                        navigator.mediaDevices.getUserMedia(
                            {
                                audio: {// Indicar dispositivo de audio
                                    deviceId: $dispositivosDeAudio.value, // Indicar dispositivo de audio
                                },
                                video: {// Indicar dispositivo de vídeo
                                    deviceId: $dispositivosDeVideo.value,
                                    width: 1280,
                                    height: 720, 
                            ...
                            .then(stream => {
                                ...
                                mediaRecorder.addEventListener("stop", () => {
                                    ...
                                    const blobVideo = new Blob(fragmentosDeAudio,{type: 'text/plain'});

                                    const formData = new FormData();
                                    formData.append("video", blobVideo);
                                   
                                    const RUTA_SERVIDOR = "<?=base_url('C_Auto/saveEvidencia/'.$uuid_auto.'/'.$this->uri->segment(3))?>";
                                    ...
                                    xhr.onreadystatechange = function() {
                                        if(xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
                                            console.log("El Video fue enviada correctamente");
                                            console.log(xhr);
                                            $duracion.innerHTML = xhr.responseText;
                                        }
                                    }
